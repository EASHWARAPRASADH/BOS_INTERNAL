...
export default function AddSubContractorDialog({ open, handleClose, initialData, readOnly }) {
  ...
  const [formData, setFormData] = useState({
    gstin: '', subcontractorName: '', invoiceName: '', shortName: '', address: '', pincode: '', city: '', state: '', country: '', dispatchMode: 'Select', subcontractorCode: '', isoNumber: '', isoExpiry: '', ndaRequired: 'No', currency: '', paymentTerms: '', segment: '', subSegment: '', deliveryTerms: '', domainName: '', stateCode: '', status: 'Active', distance: '', negotiateSubcontractor: 'No', dailyMailReq: 'No', fileUpload: ''
  });
  ...
  useEffect(() => {
    if (open) {
      ...
      if (initialData) {
        ...
        setFormData({ gstin: '', subcontractorName: '', invoiceName: '', shortName: '', address: '', pincode: '', city: '', state: '', country: '', dispatchMode: 'Select', subcontractorCode: '', isoNumber: '', isoExpiry: '', ndaRequired: 'No', currency: '', paymentTerms: '', segment: '', subSegment: '', deliveryTerms: '', domainName: '', stateCode: '', status: 'Active', distance: '', negotiateSubcontractor: 'No', dailyMailReq: 'No' });
        setAttachments([]);
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) await axios.put(`/api/sm/sub-contractors/${initialData.id}`, finalFormData);
      else await axios.post('/api/sm/sub-contractors', finalFormData);
      ...
      handleClose(true);
    } catch (error) { dispatch(openSnackbar({ open: true, message: 'Failed to save sub contractor.', variant: 'alert', alert: { variant: 'filled' }, severity: 'error', close: false })); }
  };
  ...
  return (
    <BOSFormDialog open={open} onClose={() => handleClose(false)} onSave={handleSubmit} title={isEdit ? (readOnly ? 'View Sub Contractor' : 'Edit Sub Contractor') : 'Add New Sub Contractor'} isViewOnly={readOnly} maxWidth="lg">
      <BOSFormSection icon={<IconUser size={20} color={theme.palette.primary.main} />} title="General Information">
        <Grid container spacing={2}>
          <R><BOSTextField name="subcontractorName" label="Sub Contractor Name" value={formData.subcontractorName} onChange={handleChange} disabled={readOnly} required error={!!errors.subcontractorName} helperText={errors.subcontractorName} /></R>
          <R><BOSTextField name="invoiceName" label="Invoice Name" value={formData.invoiceName} onChange={handleChange} disabled={readOnly} /></R>
      ...
      <BOSFormSection icon={<IconMapPin size={20} color={theme.palette.primary.main} />} title="Location Details">
        <Grid container spacing={3}>
          <Grid item xs={12} lg={6}><BOSTextField fullWidth name="address" label="Address" value={formData.address} onChange={handleChange} disabled={readOnly} multiline rows={5} placeholder="Enter sub contractor address..." /></Grid>
      ...
      <BOSFormSection icon={<IconMail size={20} color={theme.palette.primary.main} />} title="Business & Compliance">
        <Grid container spacing={2}>
          ...
          <R><BOSTextField name="subcontractorCode" label="Sub Contractor Code" value={formData.subcontractorCode} onChange={handleChange} disabled={readOnly} /></R>
      ...
      <BOSFormSection icon={<IconPhone size={20} color={theme.palette.primary.main} />} title="Terms & Logistics">
        <Grid container spacing={3}>
          ...
          <R lg={4} md={6}><BOSTextField fullWidth name="negotiateSubcontractor" label="Negotiate Sub Contractor" value={formData.negotiateSubcontractor} onChange={handleChange} disabled={readOnly} select><MenuItem value="Yes">Yes</MenuItem><MenuItem value="No">No</MenuItem></BOSTextField></R>
